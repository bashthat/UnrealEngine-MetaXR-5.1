...
EVisibility SOculusToolWidget::IsVisible(FName tag) const
{
	...
	EOculusXRPlatform targetPlatform = EditorSettings->PerfToolTargetPlatform;
	 
	if(targetPlatform == EOculusXRPlatform::Mobile && !((int)setting->supportMask & (int)SupportFlags::SupportMobile)) return EVisibility::Collapsed;
	if(targetPlatform == EOculusXRPlatform::PC && !((int)setting->supportMask & (int)SupportFlags::SupportPC)) return EVisibility::Collapsed;
...
void SOculusToolWidget::RebuildLayout()
{
	...
	for (uint8 i = 0; i < (uint8)EOculusXRPlatform::Length; ++i)
	{
...
void SOculusToolWidget::Construct(const FArguments& InArgs)
{
	...
	PlatformEnum = StaticEnum<EOculusXRPlatform>();
	Platforms.Reset(2);
...
void SOculusToolWidget::OnChangePlatform(TSharedPtr<FString> ItemSelected, ESelectInfo::Type SelectInfo)
{
	...
	{
		UOculusXREditorSettings* EditorSettings = GetMutableDefault<UOculusXREditorSettings>();
		EditorSettings->PerfToolTargetPlatform = (EOculusXRPlatform)idx;
...
bool SOculusToolWidget::UsingForwardShading() const
{
	...
	URendererSettings* Settings = GetMutableDefault<URendererSettings>();
	EOculusXRPlatform targetPlatform = EditorSettings->PerfToolTargetPlatform;
	return targetPlatform == EOculusXRPlatform::Mobile || Settings->bForwardShading;
...
{
	GConfig->SetBool(TEXT("/Script/OculusXREditor.OculusXREditorSettings"), TEXT("bAddMenuOption"), NewState == ECheckBoxState::Checked ? true : false, FString::Printf(TEXT("%sDefaultEditor.ini"), *FPaths::SourceConfigDir()));
	GConfig->Flush(0);
}
...
{
	bool v;
	GConfig->GetBool(TEXT("/Script/OculusXREditor.OculusXREditorSettings"), TEXT("bAddMenuOption"), v, FString::Printf(TEXT("%sDefaultEditor.ini"), *FPaths::SourceConfigDir()));
...
FReply SOculusToolWidget::SupportDashEnable(bool text)
{
	...
	Settings->UpdateSinglePropertyInConfigFile(Settings->GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UOculusXRHMDRuntimeSettings, bSupportsDash)), Settings->GetDefaultConfigFilename());
	return FReply::Handled();
}
