...
FScreenPassTexture FStreamlineViewExtension::PostProcessPassAtEnd_RenderThread(FRDGBuilder& GraphBuilder, const FSceneView& View, const FPostProcessMaterialInputs& InOutInputs)
{
	...
	const uint32 ViewID = CVarStreamlineViewIdOverride.GetValueOnAnyThread() ? 0 : (ViewInfo.ViewState ? ViewInfo.ViewState->UniqueID : 0);
	...
	if (ShouldTagStreamlineBuffers())
	{
		...
		const bool bTagSceneColorWithoutHUD = CVarStreamlineTagSceneColorWithoutHUD.GetValueOnRenderThread();
		...
		// The next two should be CVars so developers can adjust them (default values proposed by Karan)
		// They do not *need* to match what the engine is actually using (by default 10cm and infinity)
	...
	AddStreamlineDLSSGStateRenderPass(GraphBuilder, ViewID);
