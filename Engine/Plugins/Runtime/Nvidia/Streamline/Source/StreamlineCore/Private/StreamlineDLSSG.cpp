...
static void DLSSGOnBackBufferReadyToPresent(SWindow& InWindow, const FTexture2DRHIRef& InBackBuffer)
{
...
#endif
	if (!(bIsGameWindow || (CVarStreamlineEditorTagUIColorAlpha.GetValueOnRenderThread() && bIsPIEWindow)))
	{
		return;
	}
	...
	const bool bTagUIColorAlpha = CVarStreamlineTagUIColorAlpha.GetValueOnRenderThread();
	...
	if (bTagUIColorAlpha)
	{
		...
		GraphBuilder.AddPass(
			RDG_EVENT_NAME("Streamline UIColorAndAlpha Tag"),
			PassParameters,
#if (ENGINE_MAJOR_VERSION == 4) && (ENGINE_MINOR_VERSION == 25)
...
#endif
			[RHIExtensions, PassParameters, InputViewRect, ViewId](FRHICommandListImmediate& RHICmdList) mutable
			{
				check(PassParameters->UIColorAndAlpha);
				PassParameters->UIColorAndAlpha->MarkResourceAsUsed();
				FRHITextureWithRect UIColorAndAlpha = { PassParameters->UIColorAndAlpha->GetRHI(), InputViewRect };
